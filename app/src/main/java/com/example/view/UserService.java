...
public class UserService {
    public static boolean signIn(String name, String password) {
        //MyThread myThread = new MyThread("http://192.168.0.119/Teamup_web_war_exploded/SignIn",name,password);
        MyThread myThread = new MyThread("http://121.40.96.127:8080/Teamup_web_war_exploded/SignIn",name,password);
        try
        {
            myThread.start();
            myThread.join();
    ...
    public static boolean signUp(String name, String password) {
        //MyThread myThread = new MyThread("http://192.168.0.119/Teamup_web_war_exploded/SignUp",name,password);
        MyThread myThread = new MyThread("http://121.40.96.127:8080/Teamup_web_war_exploded/SignUp",name,password);
        try
        {
            myThread.start();
            myThread.join();
